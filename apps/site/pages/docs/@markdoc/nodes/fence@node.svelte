<script lang="ts">
  ...
  export let ch = false; // copy highlight
  export let cs = false; // copy steps
  export let nums = false;
  export let hl: string | null = null; // highlights
  export let style: string = ''; // ignore
  style = '';

  $: highlightLines = (hl?.split(',').map((item) => {
  ...
  $: currentHighlightedLines = cs ? stepHighlightLines : highlightLines;
  ...
  $: unescapedRawCode = code?.replace(/&#8203/g, '') ?? '';

  let showCopiedCodePrompt = false;
  async function copyCodeToClipboard() {
    try {
      const copiedCode =
        currentHighlightedLines.length > 0 && (ch || cs)
          ? unescapedRawCode
              .split('\n')
  ...
  $: topbarTitle = title ?? (lang === 'sh' ? 'terminal' : lang);
</script>
